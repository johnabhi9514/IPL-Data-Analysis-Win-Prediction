(function () {
  ...
  CuteSelect.tools = {
        ...
        for (var x = 0; x < classes.length; x++) {
          if (classes[x].selectorText != undefined) {
            ...
            if (
              selectPosition >= 0 &&
              optionPosition >= 0 &&
              (selectChar == "" || selectChar == "}" || selectChar == " ") &&
              (optionChar == "" || optionChar == "}" || optionChar == " ")
            ) {
              text = classes[x].cssText
                ? classes[x].cssText
                : classes[x].style.cssText;
              css += text
                .replace(/\boption\b/g, "[data-cuteselect-value]")
                .replace(/\bselect\b/g, "[data-cuteselect-item]");
              continue;
            }
            if (selectPosition >= 0) {
              var character = classes[x].selectorText.charAt(
                selectPosition - 1
              );
    ...
    createSelect: function (item) {
      ...
      // Get select options
      var options =
        "<div data-cuteselect-title>" +
        caption +
        "</div><div data-cuteselect-options><div data-cuteselect-options-container>";
      var cells = item.getElementsByTagName("option");
      for (var i = 0; i < cells.length; i++) {
        if (cells[i].hasAttribute("disabled")) {
          continue;
        }
        if (cells[i].hasAttribute("class")) {
          var optionStyle = ' class="' + cells[i].getAttribute("class") + '"';
        } else {
          var optionStyle = "";
        }
        if (cells[i].hasAttribute("id")) {
          var optionId = ' id="' + cells[i].getAttribute("id") + '"';
        } else {
          var optionId = "";
        }
        if (cells[i].hasAttribute("selected")) {
          options +=
            '<div data-cuteselect-value="' +
            cells[i].value +
            '" data-cuteselect-selected="true"' +
            optionStyle +
            optionId +
            ">" +
            cells[i].innerHTML +
      ...
      item.setAttribute(
        "data-cuteselect-target",
        node.getAttribute("data-cuteselect-item")
      );
    ...
    show: function (item) {
      if (item.parentNode.hasAttribute("data-cuteselect-item")) {
        var source = item.parentNode.getAttribute("data-cuteselect-item");
      } else {
        var source = item.getAttribute("data-cuteselect-item");
      }
    ...
    selectOption: function (item) {
      ...
      for (var i = 0; i < cells.length; i++) {
        if (cells[i].hasAttribute("data-cuteselect-title")) {
          cells[i].innerHTML = label;
        }
      }
    ...
    writeStyles: function () {
      toWrite =
        '<style type="text/css">' +
        CuteSelect.tools.getStyle() +
        " [data-cuteselect-options] { opacity: 0; display: none; }</style>";
  ...
  CuteSelect.event = {
    parse: function () {
      var cells = document.getElementsByTagName("select");
      for (var i = 0; i < cells.length; i++) {
        CuteSelect.tools.createSelect(cells[i]);
      }
    },
    listen: function () {
      ...
      document.onclick = function (event) {
        FIRSTLOAD = false;
        if (
          (!event.target.getAttribute("data-cuteselect-item") &&
            !event.target.getAttribute("data-cuteselect-value") &&
            !event.target.hasAttribute("data-cuteselect-title")) ||
          ((event.target.hasAttribute("data-cuteselect-item") ||
            event.target.hasAttribute("data-cuteselect-title")) &&
            SOMETHINGOPEN)
        ) {
          CuteSelect.tools.hideEverything();
        ...
        if (event.target.getAttribute("data-cuteselect-value")) {
          ...
          CuteSelect.tools.show(action);
        }
        return false;
      };
    },
    manage: function () {
      if (CuteSelect.tools.canRun()) {
        // IE Compatibility
